...
#Correlated features are not a problem for the random forest but in order to properly read the permutation features importance one has to know
#which features are highly correlated since their importance will be underestimated
#Dython allows to compute association measures for a dataframe made up by variables of diffent type
...
#Best threshold: 0.3791, Train Score: 0.7452
#opt_ths=0.3791
# endregion

# region Predicitons, score
...
result=permutation_importance(best_rf.named_steps["classifier"],X_test,y_test,random_state=123,scoring=custom_scorer)
...
#pickle.dump(fig,open("Feature_importance.pickle","wb"))
# endregion
...
#(not available in python) or resort to use rfpimp to compute group permutations (but it does not allow to use AUC, so it is useless for
# unbalanced data)
...
# region Partial dependence plot#######################################
#No internet service important
#Internet/Monthly charges important (if you have internet and lot of services you prolly not gonna drop)
#Lenght of contract
...
# region pdp_plot
pdp_tenure = pdp.pdp_isolate(model=best_rf.named_steps["classifier"], dataset=X_train, model_features=X_train.columns,
                             feature='tenure')
plot_params={"title": "Probability to drop within 1 month",
             "subtitle": ""
}

fig,ax=pdp.pdp_plot(pdp_tenure, 'Tenure',center=False,plot_params=plot_params,show_percentile=True)

ax["pdp_ax"].set_xticklabels(unscaling(ax["pdp_ax"].get_xticks(),X_train_ori.tenure))



std_ten=[]
for i in range(len(pdp_tenure.ice_lines.columns)):
  std_ten.append(pdp_tenure.ice_lines.iloc[:, i].std())


fig2,ax2=plt.subplots()
plt.plot(unscaling(ax["pdp_ax"].lines[1].get_xdata(),X_train_ori.tenure),ax["pdp_ax"].lines[0].get_ydata())
ax2.set_xlabel('Tenure (months)');ax2.set_ylabel('Probability to short term drop');

ax2.fill_between(unscaling(ax["pdp_ax"].lines[1].get_xdata(),X_train_ori.tenure), ax["pdp_ax"].lines[0].get_ydata()+std_ten, ax["pdp_ax"].lines[0].get_ydata(

)-std_ten,color='b', alpha=.1)
...
# region pdp_plot
pdp_No_internet = pdp.pdp_isolate(model=best_rf.named_steps["classifier"], dataset=X_train,
                             model_features=X_test.columns,
                             feature=["InternetService_DSL","InternetService_Fiber optic","InternetService_No"])

fig, axes = pdp.pdp_plot(pdp_isolate_out=pdp_No_internet,
                         feature_name='No internet',
...
#Montlhy charges
#Fiber optic probably has a high price compared to ohter companies offers/client do perceive it with enough value
fig, axes, summary_df =info_plots.actual_plot(model=best_rf.named_steps["classifier"], X=X_train,
...
#Fiber is costly/not good nough for that price
...
#Contract
pdp_contract = pdp.pdp_isolate(model=best_rf.named_steps["classifier"], dataset=X_train,
                             model_features=X_test.columns,
                             feature=["Contract_Month-to-month","Contract_One year","Contract_Two year"])

fig, axes = pdp.pdp_plot(pdp_isolate_out=pdp_contract, plot_pts_dist=False,
                         feature_name='Contract',center=False)
...
predict_proba=best_rf.named_steps["classifier"].predict_proba(X_test)
fpr, tpr, thresholds=roc_curve(y_test, predict_proba[:,1],pos_label="Yes")
plt.plot(fpr,tpr)
